...
ROUTE_COLUMNS = (
    ...
    "children", "ret_origin", "ret_from", "ret_to", "rolling_days",
)
...
class Storage:
    def __init__(self, path: Path | None = None) -> None:
        ...
        # Una sola vez: las 12 rutas generales pasan a ventana móvil de 10 meses
        if not self.conn.execute("SELECT 1 FROM kv WHERE k='upgrade_rolling_v1'").fetchone():
            self.conn.execute(
                "UPDATE routes SET rolling_days=300 WHERE depart_to='2027-06-30' AND rolling_days IS NULL"
            )
            self.conn.execute("INSERT OR REPLACE INTO kv (k, v) VALUES ('upgrade_rolling_v1', '1')")
        self.conn.commit()
    ...
    def feed_has_history(self, feed: str) -> bool:
        return self.conn.execute("SELECT 1 FROM feed_seen WHERE feed=? LIMIT 1", (feed,)).fetchone() is not None

    def feed_seen(self, feed: str, item_id: str) -> bool:
        return self.conn.execute(
            "SELECT 1 FROM feed_seen WHERE feed=? AND item_id=?", (feed, item_id)
        ).fetchone() is not None

    def mark_feed_item(self, feed: str, item_id: str, alerted: bool) -> None:
        self.conn.execute(
            "INSERT OR IGNORE INTO feed_seen (feed, item_id, seen_at, alerted) VALUES (?,?,?,?)",
            (feed, item_id, _utc_now(), 1 if alerted else 0),
        )
        self.conn.commit()

    def count_since(self, table: str, column: str, days: int) -> int:
        since = (date.today() - timedelta(days=days)).isoformat()
